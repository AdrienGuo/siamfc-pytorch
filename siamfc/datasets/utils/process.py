...
def resize(img, boxes, rate, interp=cv2.INTER_LINEAR):
    """
    Args:
        img (np.array)
        boxes (np.array): (N, 4)
    ...
    # 我不懂為什麽在 2080ti 這台上面會回傳 float
    new_w = int(round(img_w * rate))
    new_h = int(round(img_h * rate))
    img = cv2.resize(
        img, (new_w, new_h), interpolation=interp)
...
def z_score_norm(img):
    assert isinstance(
        img, np.ndarray), "ERROR, img type should be numpy.ndarray!!"
